...
class ImportantNotificationService extends ChangeNotifier {
  ...
  //READ NOTIFICATIONS
  Future<List<ImportantNotification>> getNotifications(String userId) async {
    ...
    try {
      ...
      data.forEach((key, value) {
        try {
          final notification = ImportantNotification.fromRawJson(jsonEncode(value));
  ...
  //SAVE NOTIFICATION
  Future<void> saveNotification(ImportantNotification notification, String userId) async {
    final url = Uri.https(_baseUrl, 'User/${userId}/notifications.json');
    try {
      final resp = await http.post(url, body: jsonEncode(notification.toJson()));
    } catch (e) {
